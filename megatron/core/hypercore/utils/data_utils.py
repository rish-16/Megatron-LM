...
def load_data(args, datapath):
    ...
    if args.dataset == 'airport' and args.use_adj_feats:
...
def load_data_lp(dataset, use_feats, data_path):
    data_pppp = data_path.replace("\r", "")

    if dataset in ['cora', 'pubmed']:
        adj, features = load_citation_data(dataset, use_feats, data_pppp)[:2]
    elif dataset == 'disease_lp':
        adj, features = load_synthetic_data(dataset, use_feats, data_pppp)[:2]
    elif dataset == 'airport':
        adj, features = load_data_airport(dataset, data_pppp, return_label=False)
    else:
...
def load_data_nc(dataset, use_feats, data_path, split_seed):
    ...
    if dataset in ['cora', 'pubmed']:
        adj, features, labels, idx_train, idx_val, idx_test = load_citation_data(
            dataset, use_feats, data_pppp, split_seed
        )
    else:
        if dataset == 'disease_nc':
            adj, features, labels = load_synthetic_data(dataset, use_feats, data_pppp)
            val_prop, test_prop = 0.10, 0.60
        elif dataset == 'airport':
            adj, features, labels = load_data_airport(dataset, data_pppp, return_label=True)
...
def load_synthetic_md_data(dataset_str, use_feats, data_path):
    ...
    idx_counter = 0
    edges = []
    path = os.path.join(data_path, "{}.edges.csv".format(dataset_str)).replace('\r', '')

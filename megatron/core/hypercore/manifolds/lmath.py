...
def _expmap(x, u, k: torch.Tensor, dim: int = -1):
    ...
    sqrt_c = torch.sqrt(k)
    p = torch.cosh(nomin / sqrt_c) * x + torch.sinh(nomin / sqrt_c) * u * sqrt_c
    return p


def expmap0(u, *, k, dim=-1, norm_control=False):
    r"""
    ...
    """
    return _expmap0(u, k, dim=dim, norm_control=norm_control)


def _expmap0(u, k: torch.Tensor, dim: int = -1, norm_control = False):
    nomin = _norm(u, keepdim=True, dim=dim).clamp_max(50)
    if norm_control:
        denom = (nomin / torch.sqrt(k)).clamp_max(1)
    else:
        denom = (nomin / torch.sqrt(k))
    l_v = torch.cosh(denom) * torch.sqrt(k)
    r_v = torch.sqrt(k) * torch.sinh(denom) * u / nomin
    dn = r_v.size(dim) - 1
    p = torch.cat((l_v + r_v.narrow(dim, 0, 1), r_v.narrow(dim, 1, dn)), dim)
    return p


def logmap(x, y, *, k, dim=-1, is_tan_normalize = False):
    r"""
    ...
    """
    return _logmap(x, y, k=k, dim=dim, is_tan_normalize=is_tan_normalize)


def _logmap(x, y, k, dim: int = -1, is_tan_normalize = False):
    dist_ = _dist(x, y, k=k, dim=dim, keepdim=True)
    nomin = y + _inner(x, y, keepdim=True) * x / k
    denom = _norm(nomin, keepdim=True)
    y_tan = dist_ * nomin / denom
    if is_tan_normalize:
        y_tan = normalize_tangent(x, y_tan, k)
    return y_tan

def normalize_tangent(p, p_tan, k):
        """
        ...
        p_head = torch.sqrt(k + torch.sum(torch.pow(p_tail, 2), dim=1, keepdim=True) + eps)
...
def logmap0(y, *, k, dim=-1, is_tan_normalize):
    r"""
    ...
    """
    return _logmap0(y, k=k, dim=dim, is_tan_normalize = is_tan_normalize)


def _logmap0(y, k, dim: int = -1 , is_tan_normalize = False):
...
def _logmap0back(x, k, dim: int = -1):
    ...
    denom = _norm(nomin, keepdim=True).clamp_min(1e-6)
    return dist_ * nomin / denom
...
def _parallel_transport0back(x, v, k, dim: int = -1):
    ...
    nom = _inner(lmap, v, keepdim=True)
    denom = (_dist0(x, k=k, dim=dim, keepdim=True) ** 2).clamp_min(1e-6)
